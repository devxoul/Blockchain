...
public struct _BigInt<Word: FixedWidthInteger & UnsignedInteger & Codable> :
  BinaryInteger, SignedInteger, CustomStringConvertible,
  CustomDebugStringConvertible, Codable
...
// MARK: - I wrote from here

public typealias BigInt = _BigInt<UInt>

extension _BigInt {
  public init(from decoder: Decoder) throws {
    let container = try decoder.singleValueContainer()
    self.init(try container.decode([Word].self))
  }

  public func encode(to encoder: Encoder) throws {
    var container = encoder.singleValueContainer()
    try container.encode(self._data)
  }
}

public func pow(_ lhs: BigInt, _ rhs: Int) -> BigInt {
  var result = lhs
  for _ in 0..<rhs {
    result *= lhs
  }
